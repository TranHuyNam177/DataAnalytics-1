...
def run(
    ...
    for f in files:
        try:
            months.append(int(get_month(f)))
        except (Exception,):  # một số thằng lưu tên không theo chuẩn
            months.append(0)
    file_name = files[months.index(max(months))]

    bad_loan_path = join(path, file_name)
    bad_loan = pd.read_excel(bad_loan_path, sheet_name='Bad loan', usecols=[0, 4])
    bad_loan.columns = ['account_code', 'group']
    ...
    debt_type = pd.read_sql(
        ...
        SELECT DISTINCT
            ...
            ELSE [margin_outstanding].[type]
            END [type],
            SUM([margin_outstanding].[principal_outstanding]) [bad_loan]
        FROM [margin_outstanding]
        ...
        AND [margin_outstanding].[date] = '{end_date}'
        GROUP BY [margin_outstanding].[account_code], [customer_name], [type]
        """,
        connect_DWH_CoSo,
        index_col='account_code'
    )
    bad_loan = bad_loan.merge(debt_type, how='outer', left_on='account_code', right_index=True)
    bad_loan = bad_loan.dropna(subset=['bad_loan'])
    bad_loan = bad_loan.sort_values('type')

    bad_loan_g = bad_loan.groupby('type')['bad_loan'].sum()
