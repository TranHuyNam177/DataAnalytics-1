...
def process_data(
    ...
    # Tìm min(max loan price, price)
    max_price = table[['max_loan_price', 'price']].min(axis=1)
    # Tìm max(mr loan ratio, dp loan ratio)
    ty_le = table[['mr_loan_ratio', 'dp_loan_ratio']].max(axis=1)
    # ML -> Max loan price
    table['%giaHoaVon_ML'] = (gia_hv - (max_price * ty_le / 100)) / (table['max_loan_price'] * ty_le / 100)
    # MP -> market price
